 ...
 * @param doc_path Path to Swagger/OpenAPI (YAML or JSON) file (required)
 ...
 * @param display_topbar // 0: topbar hidden, 1: topbar displayed without search bar, 2: topbar displayed with search bar (optional)
 */
export type SwaggerOptions =  {
    doc_path: string | any;  // Path to Swagger/OpenAPI (YAML or JSON) file  or jsdoc object (required)
    ...
    display_topbar?: 0 | 1 | 2 ;  // 0: topbar hidden, 1: topbar displayed without search bar, 2: topbar displayed with search bar (optional)
}
...
export function makeHtml(fileName:string, swaggerOptions:SwaggerOptions, route:string  ): void {
    ...
    <html lang="en">
        ...
        <script> const doc_path = "${swaggerOptions.doc_path}"; </script>
